...
def extract_cards_from_html(html_content: str) -> List[str]:
    ...
    # Telegra.ph usa <ul> y <li> para listas
    li_pattern = r'<li[^>]*>(.*?)</li>'
...
def extract_cards(text: str) -> List[str]:
    ...
    # Patrón principal: 16 dígitos, luego separador, mes (2), separador, año (2 o 4), separador, CVV (3-4)
    patterns = [
        # Formato estándar: 1234567890123456|02|2030|816
        ...
        # Formato sin separadores explícitos (riesgo de falsos positivos)
        ...
        for match in matches:
            try:
                if len(match) == 4:
                    ...
                    # Limpiar caracteres no numéricos del número de tarjeta
                ...
                logger.warning(f"⚠️ Error procesando tarjeta con patrón {pattern}: {match} - {e}")
                continue

    # También buscar patrones con prefijos comunes
    prefix_patterns = [
        r'CC[:=\s]+([\d\s|/-]{16,})[|/\s]+(\d{2})[|/\s]+(\d{2,4})[|/\s]+(\d{3,4})',
        r'Card[:=\s]+([\d\s|/-]{16,})[|/\s]+(\d{2})[|/\s]+(\d{2,4})[|/\s]+(\d{3,4})',
        r'Credit[:=\s]+([\d\s|/-]{16,})[|/\s]+(\d{2})[|/\s]+(\d{2,4})[|/\s]+(\d{3,4})',
    ]
    ...
    for pattern in prefix_patterns:
        ...
        for match in matches:
            try:
                ...
                logger.warning(f"⚠️ Error procesando tarjeta con prefijo: {match} - {e}")
                continue
...
async def process_message_links(message: Message, chat_id: int) -> int:
    """Procesa los enlaces de un mensaje (texto y botones) y retorna el número de tarjetas encontradas."""
    new_cards_count = 0
